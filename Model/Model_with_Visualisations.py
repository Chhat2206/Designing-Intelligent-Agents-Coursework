...
class RealEstateAI:
    ...
    def build_models(self):
        ...
        print(f"Valuation model MAE: ${val_mae:.2f}, RÂ²: {val_r2:.4f}")
        ...
        print(f"Rental model MAE: ${rent_mae:.2f}, RÂ²: {rent_r2:.4f}")
    ...
    def update_portfolio(self, months=1, visualize_map=False):
        ...
        self.history['capital'].append(float(self.available_capital))
        self.history['net_worth'].append(float(self.net_worth))
        self.history['monthly_cash_flow'].append(float(self.monthly_cash_flow))
        self.history['num_properties'].append(len(self.portfolio))
        self.history['total_equity'].append(float(self.total_equity))
        self.history['total_debt'].append(float(self.total_debt))
        self.history['roi'].append(float(portfolio_roi))

        # Generate map if requested and a year has passed
        if visualize_map and (initial_date.year < self.current_date.year or months >= 12):
            self.visualize_portfolio_map()
        ...
        return {
            ...
            'capital': float(self.available_capital),
    ...
    def auto_invest(self, max_properties=5, min_cash_reserve=20000, months_between_purchases=3, min_cash_flow=0, interactive_visualization=True):
        ...
        print(f"- Interactive visualization: {'Enabled' if interactive_visualization else 'Disabled'}")

        properties_acquired = 0
        simulation_months = 0
        max_simulation_months = 60  # 5 years
        
        # Track years for visualization
        last_visualization_year = None
        ...
        # Check if we already have properties to visualize at the start
        if len(self.portfolio) > 0:
            print("\nGenerating initial portfolio map visualization...")
            self.visualize_portfolio_map(wait_for_next_step=interactive_visualization)
        ...
        while properties_acquired < max_properties and simulation_months < max_simulation_months:
            ...
            # Check if we need to generate the map visualization
            current_year = self.current_date.year
            if last_visualization_year is None and simulation_months >= 12:
                # First visualization after 1 year of trading
                print("\nGenerating first annual portfolio map visualization...")
                self.visualize_portfolio_map(wait_for_next_step=interactive_visualization)
                last_visualization_year = current_year
            elif last_visualization_year is not None and current_year > last_visualization_year:
                # Annual update of visualization
                print("\nUpdating annual portfolio map visualization...")
                self.visualize_portfolio_map(wait_for_next_step=interactive_visualization)
                last_visualization_year = current_year
                ...
                if max_price <= 0:
                    ...
                    if len(opportunities) > 0:
                        ...
                        if success:
                            ...
                            # Generate visualization after property purchase
                            print("\nUpdating portfolio map visualization after purchase...")
                            self.visualize_portfolio_map(wait_for_next_step=interactive_visualization)
                        else:
            ...
            # Update portfolio for next month (without generating map here)
            self.update_portfolio(months=1, visualize_map=False)
            simulation_months += 1
        ...
        # Final visualization
        if len(self.portfolio) > 0:
            print("\nGenerating final portfolio map visualization...")
            self.visualize_portfolio_map(wait_for_next_step=interactive_visualization)
    ...
    def visualize_portfolio_map(self, save_html=True, show_plot=True, wait_for_next_step=False):
        """Create an interactive choropleth map visualization of the real estate portfolio"""
        
        
        print("Generating interactive real estate portfolio map visualization...")
        
        # Check if we have properties to visualize
        if len(self.portfolio) == 0:
            print("No properties in portfolio to visualize.")
            return None, None
        ...
        properties = pd.DataFrame(self.portfolio)
        ...
        try:
            ...
            print(f"Warning: Could not load geographic data files: {e}")
            print("Continuing with state-level visualization only.")
        ...
        # Create the state-level map figure
        fig = px.choropleth(
            state_data,
            locations='state',
        ...
        # Update layout for state map
        fig.update_layout(
            title=f"Real Estate Portfolio Map - {self.current_date.strftime('%Y-%m-%d')}",
        ...
        if len(county_properties) > 0:
            ...
            county_data['prop_count'] = county_data.apply(
                lambda row: len(county_properties[(county_properties['state'] == row['state']) & 
                                              (county_properties['county'] == row['county'])]),
                axis=1
            )
            ...
            # Create county maps for each state with properties
            for state in state_data['state'].unique():
                ...
                # Get GeoJSON for counties in this state
                ...
                # Filter GeoJSON to include only counties from this state
                ...
                # Update layout for county map
                county_fig.update_layout(
                    title=f"{state} Counties - {self.current_date.strftime('%Y-%m-%d')}",
                    coloraxis_showscale=False,
                    height=800,
                    width=1200,
                    margin={"r":0,"t":50,"l":0,"b":0},
                ...
                county_fig.update_traces(
                    hovertemplate="<b>%{customdata[0]} County</b><br>%{customdata[1]}<extra></extra>"
                )
                
                # Store the county figure
                state_county_figs[state] = county_fig
        
        # Add legend to the state figure
        fig.add_annotation(
        ...
        app.layout = html.Div([
            ...
            html.Div([
                ...
                html.Button("Next Step", id="next-step-button", 
                            style={
                                'marginBottom': 20, 
                                'display': 'block' if wait_for_next_step else 'none',
            ...
            # Status message
            html.Div(id='status-message', style={'textAlign': 'center', 'marginTop': 20})
        ])
        
        # Existing callback for map interaction
        @app.callback(
        ...
        # Add callback for Next Step button
        @app.callback(
            Output('status-message', 'children'),
            Input('next-step-button', 'n_clicks'),
            prevent_initial_call=True
        )
        def on_next_step(n_clicks):
            if n_clicks is not None and n_clicks > 0:
                # Signal that the button was clicked
                next_step_event.set()
                return "Continuing simulation..."
            return "Simulation paused. Click 'Next Step' to continue..."
        ...
        if show_plot:
            ...
            dash_thread.daemon = True  # This ensures the thread will exit when the main program exits
            dash_thread.start()
            
            # If waiting for next step is enabled, wait for the button click
            if wait_for_next_step:
                print("Simulation paused. Click 'Next Step' button in the browser to continue...")
                next_step_event.wait()  # This will block until next_step_event.set() is called
                print("Continuing simulation...")
        
        return fig, state_county_figs




# Main simulation function
def run_real_estate_simulation(data_path, initial_capital=200000, num_properties=5, min_cash_flow=100, interactive_visualization=True):
